...
- TXT 书架：每个用户拥有独立书架空间，读取自己名下的 `novel/<用户名>/` 小说文件，并在页面中切换阅读。
- 智能章节解析：支持常见的“第 X 章 / 第 X 回 / 卷 / 序章 / 番外”等章节标题格式。
- 分页阅读：每次加载固定字数，支持章节和页码切换。
- 阅读进度：按用户保存每本书的章节、页码、总章节数和阅读进度；刷新页面后会自动打开当前用户上次阅读的小说章节。
...
1. 将 `.txt` 小说放入当前用户的 `novel/<用户名>/` 目录，或在页面中上传 TXT 文件。
2. 打开 `http://127.0.0.1:6066`。
3. 选择小说开始阅读。
4. 阅读器会自动保存当前用户的书架和阅读进度，刷新页面后会自动回到该用户上次阅读的章节。
...
如果当前系统 Python 没有安装依赖，但项目目录中已有 `.venv`，可先激活虚拟环境后再运行测试。

## 注意事项

- `reading_history.json` 是本地阅读进度文件，默认不会提交到仓库。
- 每个用户的上传图书保存在独立目录中；旧版根目录 `novel/*.txt` 会自动迁移到 `default` 用户书架。
